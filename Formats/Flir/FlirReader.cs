...
namespace MetadataExtractor.Formats.Flir
{
    public class FlirReader : IJpegSegmentMetadataReader
    {
        ...
        public async IAsyncEnumerable<Directory> ReadJpegSegmentsAsync(IAsyncEnumerable<JpegSegment> segments)
        {
            ...
            await foreach (var directory in ExtractAsync(new ByteArrayReader(buffer)))
            {
                yield return directory;
            }
        }

        public async IAsyncEnumerable<Directory> ExtractAsync(IndexedReader reader)
        {
            ...
            for (int i = 0; i < indexCount; i++)
            {
                ...
                _ = reader.GetUInt16(indexOffset + 2);
                _ = reader.GetUInt32(indexOffset + 4);
                _ = reader.GetUInt32(indexOffset + 8);
                ...
                else if (mainType == FlirMainTagType.BasicData)
                {
                    ...
                    var tm = await dateTimeReader.GetUInt32Async();
                    var ss = await dateTimeReader.GetUInt32Async() & 0xffff;
                    var tz = await dateTimeReader.GetInt16Async();
